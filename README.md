...
Если вы кладёте `.stl`-файлы в `public/models/1РЛ131Р`, они автоматически попадают в каталог при старте приложения.
Если модель загружена через админку, она сохраняется сразу в `public/models/1РЛ131Р/<название_раздела>/...` и тоже переживает перезапуск.

- папка первого уровня внутри `public/models/1РЛ131Р` становится названием раздела
- все `.stl` внутри этой папки попадают в этот раздел
- `.stl`-файлы вне `public/models/1РЛ131Р` игнорируются
